...
import { runBrowserJourney } from "./executor.js";

const [packagePath = "customers/customer-alpha", journeyId = "lead-capture", artifactRoot = "generated/browser"] = process.argv.slice(2);
const loaded = await loadBrowserPackage(resolve(packagePath));
const journey = resolveJourney(loaded, journeyId);
const site = await startSyntheticSite(loaded, { runNamespace: `browser-${Date.now()}` });

try {
  const report = await runBrowserJourney(journey, site, {
    artifactRoot: resolve(artifactRoot),
    runNamespace: site.runNamespace,
    browser: (process.env.TESTY_BROWSER as "chromium" | "firefox" | "webkit" | undefined) ?? "chromium",
