...
export function compileRequestMatchers(
  ...
  for (const [field, expression] of Object.entries(values).sort(([a], [b]) => a.localeCompare(b))) {
    ...
    switch (location) {
      ...
      case "body":
        if (name !== "raw") {
          addUnsupported(field, context, issues, "Only body.raw is supported by compiler v0.2.");
        } else {
      ...
      case "json":
        if (name.length === 0) {
          addUnsupported(field, context, issues, "JSON matcher field must include a path.");
        } else {
          bodyMatchers.push({
  ...
  return {
    ...
    ...(bodyMatchers.length === 1
      ? { requestBody: bodyMatchers[0] as Readonly<Record<string, unknown>> }
      : bodyMatchers.length > 1
