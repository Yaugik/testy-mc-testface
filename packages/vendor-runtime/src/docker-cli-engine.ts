...
export class DockerCliContainerEngine implements ContainerEngine {
  public async run(spec: ContainerRunSpec): Promise<ContainerHandle> {
    ...
    if (spec.networkName) args.push("--network", spec.networkName);
    ...
    if (id.length === 0) throw new Error("Docker did not return a container ID.");
    return { id };
  }

  public async resolveHostPort(containerId: string, containerPort: number): Promise<number> {
    const { stdout } = await runDocker(["port", containerId, `${containerPort}/tcp`]);
    const firstLine = stdout.trim().split(/\r?\n/u)[0];
    if (!firstLine) {
      throw new Error(`Docker did not expose a host port for ${containerId}:${containerPort}.`);
    }
    const match = /:(\d+)$/u.exec(firstLine);
    if (!match) throw new Error(`Unable to parse Docker port output '${firstLine}'.`);
...
async function runDocker(
  ...
  try {
    ...
    if (tolerateFailure) return { stdout: "", stderr: getErrorText(error) };
