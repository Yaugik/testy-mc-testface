...
describe("target adapter contract", () => {
  it("isolates two prepared tenant fixtures", async () => {
    ...
    await adapter.configureVendorEndpoints(left, { ipinfo: "http://run-left.vendor.test/ipinfo" });
    ...
    expect(outcome.visibleTenantIds).not.toContain(leftPrepared.controlTenantId);
  });

  it("registers an existing gateway route lease after a retry", async () => {
    const binding: GatewayRouteBinding = {
      routeId: "route-retry",
      runId: "run-retry" as RunId,
      proxyBaseUrl: "http://gateway.test/v1/proxy/route-retry",
    ...
    const context: ScenarioActionContext = {
      ...
      registerResourceLease: async () => {
        leaseAttempts += 1;
        if (leaseAttempts === 1) throw new Error("transient persistence failure");
        return {
          leaseId: "lease-retry",
          runId: binding.runId,
    ...
    }, context)).rejects.toThrow(/transient/u);
    await expect(action({
      targetOrigin: "http://target.test",
      syntheticIp: "198.51.100.10",
    }, context)).resolves.toMatchObject({ routeId: binding.routeId });
