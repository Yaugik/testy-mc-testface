...
export class PostgresScenarioRunRepository implements ScenarioRunRepository {
  ...
  public async createRun(run: PersistedRun): Promise<void> {
    await this.pool.query(
      ...
      [
        ...
        run.target,
        run.status,
        run.outcomeStatus ?? null,
        run.resolvedScenarioHash,
        JSON.stringify(run.resolvedScenario),
    ...
    const result = await this.pool.query(
      `UPDATE test_runs
       SET cancel_requested_at = COALESCE(cancel_requested_at, $2),
           updated_at = NOW()
       WHERE id = $1
         AND status NOT IN ('PASSED', 'FAILED', 'CANCELLED', 'CLEANUP')`,
      [runId, requestedAt],
    );
  ...
  public async appendTimeline(record: ScenarioTimelineRecord): Promise<void> {
    await this.pool.query(
      `INSERT INTO timeline_events (
        run_id, occurred_at, category, name, metadata
  ...
  public async listSteps(runId: RunId): Promise<readonly ScenarioStepRecord[]> {
    const result = await this.pool.query<StepRow>(
      `SELECT run_id, step_id, kind, phase, status, attempt,
              started_at, completed_at, duration_ms, output_fingerprint, error
       FROM run_steps
       WHERE run_id = $1
       ORDER BY started_at ASC, step_id ASC, attempt ASC`,
      [runId],
    );
    return result.rows.map((row) => ({
      runId: row.run_id as RunId,
      ...
      ...(row.output_fingerprint === null
        ? {}
        : { outputFingerprint: row.output_fingerprint.trim() }),
  ...
  public async buildReport(runId: RunId): Promise<ScenarioRunReport | undefined> {
    ...
    const [steps, timeline, artifacts] = await Promise.all([
      this.listSteps(runId),
      this.listTimeline(runId),
      this.listArtifacts(runId),
    ]);
    return { run, steps, timeline, artifacts };
  }
}
