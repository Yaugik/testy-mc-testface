...
export async function startReferenceSut(
  ...
  const server = createServer((request, response) => {
    void handleRequest(request, response, {
      ...
      runs,
    }).catch(() => sendJson(response, 500, { error: "internal-error" }));
  });
...
async function handleRequest(
  ...
  const trackingMatch = /^\/test-support\/v1\/runs\/([^/]+)\/tracking\.js$/u.exec(
    url.pathname,
  );
  if (request.method === "GET" && trackingMatch) {
    const run = context.runs.get(decodeURIComponent(trackingMatch[1] ?? ""));
    if (!run) return sendJson(response, 404, { error: "run-not-found" });
    response.statusCode = 200;
    response.setHeader("content-type", "application/javascript; charset=utf-8");
    response.setHeader("cache-control", "no-store");
    response.end(
      `fetch(${JSON.stringify(`/test-support/v1/traffic/idempotent?targetRunId=${encodeURIComponent(run.targetRunId)}`)},{method:"POST",headers:{"content-type":"application/json","idempotency-key":${JSON.stringify(`tracking-${run.targetRunId}`)}},body:JSON.stringify({eventId:${JSON.stringify(`tracking-${run.targetRunId}`)}})});`,
    );
    return;
  }

  const trafficMatch = /^\/test-support\/v1\/traffic\/([^/]+)$/u.exec(url.pathname);
  if (trafficMatch) {
    await handleTraffic(
      trafficMatch[1] ?? "",
      request,
  ...
  if (request.method === "POST" && url.pathname === "/test-support/v1/runs") {
    ...
    if (!run) {
      run = {
        targetRunId,
        platformRunId,
        tenantId: `tenant-primary-${fingerprint(platformRunId).slice(0, 12)}`,
        controlTenantId: `tenant-control-${fingerprint(platformRunId).slice(0, 12)}`,
        siteId: `site-${fingerprint(platformRunId).slice(0, 12)}`,
  ...
  if (request.method === "PUT" && suffix === "site") {
    run.site = await readJson(request);
    response.statusCode = 204;
    response.end();
    return;
  }
  ...
  if (request.method === "GET" && suffix === "outcome") {
    ...
    if (context.mutations.leakToControlTenant) {
      visibleTenantIds.push(run.controlTenantId);
    }
    return sendJson(response, 200, {
      targetRunId,
      visibleTenantIds,
...
async function handleTraffic(
  ...
  if (operation === "retry") {
    run.retryAttempts += 1;
    if (run.retryAttempts < 3) {
      return sendJson(response, 503, { error: "transient-unavailable" });
    }
  }
...
async function executeProviderFlow(
  ...
  for (const [vendor, path] of calls) {
    if (skipEnrichment && vendor !== "ipinfo") continue;
    const endpoint = run.vendorEndpoints[vendor];
    if (!endpoint) continue;
    const response = await fetchImpl(new URL(path, ensureTrailingSlash(endpoint)), {
      headers: { "x-testy-correlation-id": `${run.platformRunId}:${vendor}` },
      redirect: "manual",
    });
    run.providerCalls.push(vendor);
    if (!response.ok) {
      throw new Error(`Reference provider '${vendor}' returned HTTP ${response.status}.`);
    }
